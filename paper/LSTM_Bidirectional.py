...
#%%
for i_count in range(200):
    #i_count = 0
    count = 0

    if (0<= i_count <= 100):
        Test_count = str(200*count + i_count)
        epoch = 70
        learning_rate = 0.001
    else:
        Test_count = str(200 * count + i_count)
    ...
    #%%
    f = open(".\\Bidirectional\\Output\\"+pred_num+"_RUL_Predtopred_Bidirectional_"+Test_count+".csv", 'w',  newline='')
    writer = csv.writer(f)
    ...
    # define model
    model = Sequential()
    model.add(Bidirectional(LSTM(Hidden_node, activation = activation), input_shape=(n_steps, n_features)))
    ...
    model.save(".\\Bidirectional\\Model\\Bidirectional_" + Test_count + ".h5")
    ...
    plt.savefig(".\\Bidirectional\\Loss\\RUL_Predtopred_Bidirectional_Loss_" + Test_count + ".png")
    #plt.show()
    plt.close('all')
    ...
    f.close()
    #%%
    f = open(".\\Bidirectional\\Output\\" + pred_num + "_RUL_Predtopred_Bidirectional_variables.csv", 'a')
    HC = pd.read_csv(".\\Bidirectional\\Output\\"+pred_num+"_RUL_Predtopred_Bidirectional_"+Test_count+".csv")
    ...
    plt.savefig(".\\Bidirectional\\Graph\\RUL_Predtopred_Bidirectional_Graph" + Test_count + ".png")
    #plt.show()
    plt.close('all')
