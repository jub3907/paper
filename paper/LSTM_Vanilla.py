...
for i_count in range(200):
    ...
    if (0<= i_count <= 100):
        ...
        Hidden_node = 16
    #%%
    training_threshold = threshold = 0.9251785
    ...
    #%%
    f = open(".\\Vanilla\\Output\\Sample"+pred_num+"_HI_Predtopred_Vanilla_"+Test_count+".csv", 'w',  newline='')
    writer = csv.writer(f)
    ...
    # define model
    model = Sequential()
    model.add(LSTM(Hidden_node, activation = activation ,input_shape=(n_steps, n_features)))
    ...
    model.save(".\\Vanilla\\Model\\Vanilla_" + Test_count + ".h5")
    ...
    plt.savefig(".\\Vanilla\\Loss\\RUL_Predtopred_Vanilla_Loss_" + Test_count + ".png")
    #plt.show()
    plt.close('all')
    ...
    f.close()
    #%%
    f = open(".\\Vanilla\\Output\\" + pred_num + "_HI_Predtopred_Vanilla_variables.csv", 'a')
    HC = pd.read_csv(".\\Vanilla\\Output\\Sample"+pred_num+"_HI_Predtopred_Vanilla_"+Test_count+".csv")
    ...
    plt.savefig(".\\Vanilla\\Graph\\RUL_Predtopred_Vanilla_Graph" + Test_count + ".png")
    #plt.show()
    plt.close('all')
